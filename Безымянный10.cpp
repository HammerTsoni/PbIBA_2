...
{	
	...
	if (Dir % rand()%60 == 0 and health > 0) movementDirection = rand()%4;
	...
	*/	
	...
	if (x > 1100) movementDirection = 0;
	if (x < 100) movementDirection = 3;
	if (y > 600) movementDirection = 2;
	if (y < 100) movementDirection = 1;
...
void Predator::Hunt() // Пока что определение цвета вокруг
{
	for (int i = 0; i < fieldOfView; i++) {
		for (int j = 0; j < fieldOfView; j++) {
			getpixel(x - fieldOfView/2 + i, y - fieldOfView/2 + j);
			//cout<<" , x - fieldOfView/2 + i = "<< x - fieldOfView/2 + i <<", y - fieldOfView/2 + j = "<< y - fieldOfView/2 + j <<"\n";
		}
